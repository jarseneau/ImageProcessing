...
public class PixelTest {
  ...
  @Test
  public void testMainChannels() {
    IPixel rgb = new RGBPixel(10, 10, 10, 255);
    String[] correctChannels = new String[]{"red", "green", "blue"};
    String[] channels = rgb.getMainChannels();
    for (int i = 0; i < channels.length; i++ ) {
      assertEquals(correctChannels[i], channels[i]);
    }
    IPixel alpha = new AlphaPixel(10, 10, 10, 255, 255);
    correctChannels = new String[]{"red", "green", "blue", "alpha"};
    channels = alpha.getMainChannels();
    for (int i = 0; i < channels.length; i++ ) {
      assertEquals(correctChannels[i], channels[i]);
    }
  }
